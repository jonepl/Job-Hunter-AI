"""ZipRecruiter scraper adapter — fetches job listings via the JSearch API."""
...
class ZipRecruiterScraper(ScraperPort):
    """Fetches ZipRecruiter job listings via the JSearch API."""
    ...
    async def fetch_jobs(
        ...
        """Fetch job listings from ZipRecruiter via JSearch API.
        ...
        try:
            logger.info("ZipRecruiter — querying JSearch API")
            ...
            for item in data[:limit]:
                try:
                    ...
                    logger.warning("ZipRecruiter — failed to parse job item: %s", exc)
                    continue
